...
class SuperlotoController extends Controller
{
    ...
    public function super_kuponlari_getir()
    {
        $u_id = auth()->user()->id;
    ...
    {
        $u_id = auth()->user()->id;
        $listele = LotoSepetim::whereRaw('sans_turu_id= ? and kupon_no= ? and kullanici_id=? ',array(1,$key,$u_id))
            ->orderBy('id')
            ->get();
        return response()->json($listele);
    }
    ...
    public function store(SuperLotoRequest $request)
    {
        $user_id = auth()->user()->id;
        $user = Kullanici::select('user_tl','user_lotopuan')->where('id',$user_id)->get();
        $loto_turu =SansTuru::select('kolon_tl','kolon_lotopuan')->where('id',1)->get();
        $odeme_turu =KullaniciDetay::select('onay_tl','onay_lotopuan')->where('kullanici_id',$user_id)->get();

        $saydir = LotoSepetim::where([ ['sans_turu_id','=',1],['kullanici_id','=',$user_id] ]) ->get();
        ...
        if (($odeme_turu[0]['onay_tl']==1 && $user[0]['user_tl'] > 0) || ( $odeme_turu[0]['onay_lotopuan']==1 && $user[0]['user_lotopuan'] > 0 ) ) {
               $data['kullanici'] = Kullanici::find($user_id);
                ...
                if(count($saydir)%8 == 0){
                    $yeni_kupon_no = 'Su-'.Str::random(8);
                }else{
                    $bos_kupon_sayisi = LotoSepetim::select(DB::raw('count(*) as saydir, kupon_no'))
                        ->where([ ['sans_turu_id','=',1],['kullanici_id','=',$user_id],['kupon_uret','=',0] ])
                        ->groupBy('kupon_no')
                        ->get();
