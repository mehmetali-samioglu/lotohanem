...
use App\Http\Requests\SayisalLotoRequest;
...
class SayisallotoController extends Controller
{
    ...
    public function sayisal_kuponlari_getir()
    {
        $u_id = auth()->user()->id;

        $kuponss = DB::table('lotosepeti')
            ->where([
                ['lotosepeti.sans_turu_id','=', 2 ],
    ...
    {
        $u_id = auth()->user()->id;
        $listele = LotoSepetim::whereRaw('sans_turu_id= ? and kupon_no= ? and kullanici_id=?',array(2,$key,$u_id))->orderBy('id')->get();
        return response()->json($listele);
    }


    public function store(SayisalLotoRequest $request)
    {

        $user_id = auth()->user()->id;
        $user = Kullanici::select('user_tl','user_lotopuan')->where('id',$user_id)->get();
        $loto_turu =SansTuru::select('kolon_tl','kolon_lotopuan')->where('id',2)->get();
        $odeme_turu =KullaniciDetay::select('onay_tl','onay_lotopuan')->where('kullanici_id',$user_id)->get();

        $saydir = LotoSepetim::where([ ['sans_turu_id','=',2],['kullanici_id','=',$user_id] ]) ->get();
        ...
        if( ($odeme_turu[0]['onay_tl']==1 && $user[0]['user_tl'] > 0) || ( $odeme_turu[0]['onay_lotopuan']==1 && $user[0]['user_lotopuan'] > 0 )  ){
            ...
            $sirala = array($request->kolon1, $request->kolon2, $request->kolon3, $request->kolon4, $request->kolon5, $request->kolon6);
             sort($sirala);
            for ($x = 0; $x < 6; $x++) {
                echo $sirala[$x];
            }
            ...
            if(count($saydir)%8 == 0){
                $yeni_kupon_no = 'Sa-'.Str::random(8);
            }else{
                $bos_kupon_sayisi = LotoSepetim::select(DB::raw('count(*) as saydir, kupon_no'))
                    ->where([ ['sans_turu_id','=',2],['kullanici_id','=',$user_id],['kupon_uret','=',0] ])
                    ->groupBy('kupon_no')
                    ->get();
                $yeni_kupon_no = $bos_kupon_sayisi[0]['kupon_no'];
            }

            $son_hafta_no = Sonuc::select('id')->where('kapandi_mi','=',0)->orderBy('id','desc')->get();


            $data['sonuca_kolon_ekle'] = LotoSepetim::create([
                'kullanici_id' => auth()->user()->id,
                'sans_turu_id' => 2,
                'hafta_id' => $son_hafta_no[0]['id'],
                'kupon_aktif_mi' => 1,
                'kupon_uret' => 0,
