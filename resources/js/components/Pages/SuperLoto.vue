          ...
          <div class="panel-body" style="padding: 15px">
            <div class="panel widget light-widget panel-bd-left">
              <div class="panel-body" style="padding: 10px">
                <a
                  class="btn vd_btn vd_round-btn btn-xs vd_bg-blue p-0 "
                  style="margin-right:3px"
                  @click="super_topu_sil(i)"
            ...
            <div class="pd-5 text-center">
              <a
                ...
                id="sayilar"
                @click="super_topu_ekle(sayi)"
                v-for="sayi in 54"
                v-if="(sayilar.length)!=6"
                class="btn vd_btn vd_round-btn btn-xs vd_bg-blue top p-0 "
                style="margin: 4px;"
              >{{sayi}}</a>
    ...
    <div class="col-md-3">
      <div class="panel widget">
        <div class="panel-heading vd_bg-red ">
          <h3 class="panel-title text-center "> <span class="menu-icon"> <i class="fa fa-shopping-cart"></i></span>Sepetteki Süper Loto Kolonları </h3>
          <h3 class="panel-title text-center" v-if="allSuperSepet!=0">Kupon No: <b>{{sonSuperKuponNo}}</b>  </h3>
          <div class="vd_panel-menu">
            <div   data-action="minimize" data-original-title="Minimize" data-toggle="tooltip" data-placement="bottom" class=" menu entypo-icon "><i class="icon-minus3"></i></div>
            <div @click="allSuperSepet" data-action="refresh" data-original-title="Refresh" data-toggle="tooltip" data-placement="bottom" class=" menu entypo-icon smaller-font"><i class="icon-cycle"></i></div>
          </div>
          <!-- vd_panel-menu -->
        </div>
          ...
          <div class="content-list content-image menu-action-right">
            <div data-rel="scroll"  >
              <transition-group class="list-wrapper" style="padding: 10px;"  name="bounceLeft" tag="ul" enter-active-class="animated bounceInLeft" leave-active-class="animated bounceOutRight">
                <li style="margin:2px"  v-for="(i,index) in allSuperSepet"   v-bind:key="i.id">
                  <div  style="margin-left:15px"  >
                    <a  class=" btn vd_btn vd_round-btn btn-xs vd_bg-blue mgr-5" >
                      {{i.kolon1}}
                    </a>
                    <a class=" btn vd_btn vd_round-btn btn-xs vd_bg-blue mgr-5" >
                      {{i.kolon2}}
            ...
            <form action="#">
              <div class="btn-group btn-group-justified mgbt-xs-5 btn-group-xs" v-if="allSuperSepet.length==8"  >
                <a role="submit" class="btn btn-default vd_white vd_bg-green"  @click="sepeti_kuponlastir(sonSuperKuponNo)"> <i class="fa fa-check"></i>Kuponu Yatır</a>
<!--                <a role="button" class="btn btn-default vd_white vd_bg-red"> <i class="fa fa-times"></i> Kuponu İptal Et</a>-->
    ...
    <div class="col-md-6">
      <div class="tabs widget">
        <ul class="nav nav-tabs widget vd_bg-red">
          <li class="active">
            <a data-toggle="tab" href="#list-tab">
              <span class="menu-icon"><i class="fa fa-user"></i></span>
              Süper Loto <b>Kupon</b> Listesi
            ...
            <div class="content-grid column-xs-1 column-sm-4 column-md-4 column-lg-4  height-xs-2 " >
              <div data-rel="scroll" >
                <ul class="list-wrapper" >
                  <li v-for="(kupon,ix) in allSuperKupons" @click="modalAc(kupon.kupon_no,ix)"  >
                    <a href="#">
...
<script>
  ...
  export default {
    ...
    created(){
      this.fetchSuperSepet();
      this.fetchSuperKupons();
      this.fetchSonKuponNo();
      console.log(this.allSuperKupons);
    },
    computed:{
      ...mapGetters(['allSuperSepet','allSuperKupons','sonSuperKuponNo']),
    },
    mounted(){
    },
    methods: {

      ...mapActions(['fetchSuperSepet','fetchSuperKupons','fetchSuperlotoKupon','fetchSonKuponNo']),
      ...
      //Super Loto Kuponu Ekle-Çıkar-Sırala
      super_topu_ekle(sayi) {
        let vm = this;
        if (this.sayilar != "") {
      ...
      sepete_kolonlari_ekle(){
        let vm = this;
        if(this.allSuperSepet.length >=8 ) {
          ...
          axios.post('/lotohanem/superloto/kolon-ekle',this.cardList)
            .then((response)=> {
            ...
            .catch( (error) => {
              this.errors='Superloto Gönderilirken Hata Oluştu.';
          ...
          buttons: [
            {
              ...
              {
                axios.delete('/lotohanem/superloto/kolonsil/'+i['id'])
                  .then((response) => {
                    if(response.data['ok'] == true){
                      ...
                      this.allSuperSepet.splice(del,1);
                  ...
                  .catch((error)=>{
                    console.log('HATA! Kupon Silinenemedi');
                  })
              }
            },
            {
      ...
      sepeti_kuponlastir(kupon_no){
        let vm = this;
        axios.post('/lotohanem/superloto/kuponlastir/'+kupon_no)
          .then((response)=>{
            if(response.data['ok']){
              this.fetchSuperSepet();
              this.fetchSonKuponNo();
